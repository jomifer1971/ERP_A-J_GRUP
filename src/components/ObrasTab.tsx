      ...
      <div className="lg:col-span-4 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-5 md:p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-800 font-mono font-bold bg-slate-100 px-2 py-0.5 rounded-md">ADMINS</span>
          </div>
            ...
            <form onSubmit={handleCreateObra} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input 
                  ...
                  className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-slate-300/20"
                />
              </div>
              ...
              <div className="flex flex-col gap-1 relative">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest font-mono flex items-center justify-between">
                  ...
                  {isSearchingAddress && <RefreshCw className="w-2.5 h-2.5 text-slate-800 animate-spin" />}
                </label>
                <div className="relative">
                  <input 
                    ...
                    className="w-full pl-8 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-slate-300/20 font-medium"
                  />
                ...
                {showAddressDropdown && addressSuggestions.length > 0 && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-100 rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto overflow-hidden divide-y divide-gray-50">
                    {addressSuggestions.map((item, idx) => (
                      ...
                      >
                        <MapPin className="w-3 h-3 text-slate-800 mt-0.5 shrink-0" />
                        <span className="truncate">{item.display_name}</span>
                      </button>
                    ))}
                  </div>
                )}
                <p className="text-[9px] text-slate-700 bg-slate-50/80 p-2 rounded-lg leading-relaxed mt-1">
              ...
              <button
                type="submit"
                className="w-full mt-2 bg-slate-800 hover:bg-slate-700 text-white py-2.5 rounded-xl text-xs font-bold font-mono uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer shadow-sm active:scale-95 transition-transform"
              >
            ...
            {obras.map(o => (
              <div 
                ...
                className={`p-3.5 rounded-2xl border text-left cursor-pointer transition-all duration-150 relative ${
                  selectedObraId === o.id 
                    ? 'bg-slate-100/60 border-slate-400 shadow-sm ring-1 ring-slate-400/30' 
                    : 'bg-gray-50 border-gray-200 hover:bg-gray-100/50'
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  <h4 className="font-bold text-xs truncate max-w-[200px]" style={{ color: selectedObraId === o.id ? '#1e293b' : '#334155' }}>
                    {o.nombre}
                  </h4>
        ...
        <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-5 md:p-6 flex flex-col gap-5">
          ...
          {currentObra ? (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 p-4 bg-gradient-to-r from-teal-50/40 to-cyan-50/20 rounded-2xl border border-teal-150">
                <div className="flex items-start gap-3">
                  <div className="p-3 bg-white border border-gray-200 text-slate-805 rounded-xl flex items-center justify-center shadow-xs">
                    <Building className="w-6 h-6" />
                  </div>
                  <div>
                    <h2 className="text-base font-bold text-gray-800 leading-tight">{currentObra.nombre}</h2>
                    <p className="text-[11px] text-slate-700 font-mono mt-1 flex items-center gap-1 font-bold">
              ...
              <div id="geovalla_container" className="bg-[#f8fafc] rounded-2xl border border-gray-200/90 p-4 flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2.5 border-b border-gray-100">
                  <div className="flex items-center gap-2">
                    <div className="p-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-800">
                      <Shield className="w-4 h-4 text-slate-808" />
                    </div>
                    <div>
                ...
                {isAdmin ? (
                  <form onSubmit={handleSaveGeovalla} className="flex flex-col gap-3 mt-1.5 bg-white p-3 rounded-xl border border-[#e2e8f0]">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black font-mono text-slate-800 uppercase">Modo de Modificación (Panel CEO & Admin)</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-1">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select 
                          ...
                          className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs font-bold font-mono text-slate-800"
                        >
                    ...
                    <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 mt-2 pt-2 border-t border-gray-100">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50/50 hover:bg-slate-100 text-[10px] text-slate-800 font-black font-mono transition-colors disabled:opacity-50"
                      >
                      ...
                      <div className="flex items-center gap-2 self-end">
                        ...
                        <button
                          type="submit"
                          className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-white text-[10px] font-black font-mono uppercase tracking-wider rounded-lg transition-all shadow-sm"
                        >
                          Guardar Geovalla
                        </button>
                      </div>
              ...
              <div className="flex flex-col gap-4 mt-1">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-gray-100 pb-3">
                  <div>
                    <h3 className="text-sm font-black font-mono uppercase tracking-wide flex items-center gap-1.5 text-gray-800">
                      <FolderOpen className="w-4 h-4 text-slate-800" /> Archivos y Planos de Obra
                    </h3>
                ...
                <div 
                  ...
                  className={`border-2 border-dashed rounded-3xl p-6 flex flex-col items-center justify-center gap-3 transition-all ${
                    isDraggingDoc 
                      ? 'border-slate-850 bg-slate-50/40 scale-[0.99] shadow-inner' 
                      : 'border-gray-200 bg-gray-50/50 hover:bg-gray-50/90'
                  }`}
                >
                  <UploadCloud className="w-10 h-10 text-slate-658 animate-pulse" />
            ...
            <div className="py-12 text-center border-2 border-dashed rounded-3xl flex flex-col items-center justify-center gap-3">
              <Building className="w-12 h-12 text-slate-200 animate-pulse" />
              <div>
              ...
              <div className="flex bg-gray-100 p-1 rounded-xl">
                <button
                  ...
                  className={`flex-1 text-center py-1.5 text-[10px] font-bold rounded-lg transition-all ${
                    calcTipo === 'yeso_pladur' ? 'bg-slate-800 text-white shadow-xs' : 'text-gray-600 hover:text-gray-800'
                  }`}
                >
                  TABIQUERÍA PLADUR
                </button>
                <button
                  ...
                  className={`flex-1 text-center py-1.5 text-[10px] font-bold rounded-lg transition-all ${
                    calcTipo === 'hormigon' ? 'bg-slate-800 text-white shadow-xs' : 'text-gray-600 hover:text-gray-800'
                  }`}
                >
                  SOLERAS HORMIGÓN
                </button>
                <button
                  ...
                  className={`flex-1 text-center py-1.5 text-[10px] font-bold rounded-lg transition-all ${
                    calcTipo === 'pintura' ? 'bg-slate-800 text-white shadow-xs' : 'text-gray-600 hover:text-gray-800'
                  }`}
                >
                  PINTADO PAREDES
                </button>
              ...
              {currentObra && (
                <div className="mt-1 pt-2 border-t border-gray-200/50 flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="text-[9px] font-black text-slate-700 hover:text-slate-900 font-mono tracking-wider uppercase border border-slate-300 px-2 py-1 rounded-md bg-white cursor-pointer active:scale-95"
                  >
